...
Users manage rules at `/notification-settings`: rules are grouped by action type with an inline per-rule enable toggle and delete, and a MaryUI modal to add/edit a rule (notify-after-minutes, notify-from start/end, required title, optional description, both with placeholders, enabled, child selector, and type-specific condition selectors). Rule deletion confirms via a **styled modal** matching the medications-page pattern (fixed overlay + `bg-base-100` card, ghost Cancel / error Delete), **not** `wire:confirm`: the trash button calls `promptDeleteRule($id)` (sets `deletingRuleId`, which renders the modal), the modal's Delete button calls `deleteRule($id)` (also resets `deletingRuleId`), and Cancel/backdrop call `closeDeleteModal()`. Default rules (Eat: 180 min from start, "Time to eat!"; Sleep: 60 min from start, "Time to wake your baby up!") are seeded via migration. No defaults for Temperature/Medication.
